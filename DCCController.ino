...
typedef struct DCCPACKET{ //packets definieren in void MAKEPACKETS?
  int LOOPS = 0; //aantal keren dat moet worden doorlopen
  int ADRES = 0; // humaan dcc adres
  boolean STATE = false; //accessoire aan of uit, links of rechts
}
DCCPACKETS;
...
void MAKEPACKETS() { //gebruikt is ontwikkeling om voorbeeld packets te maken.
    ...
    DCCPACKET[0].LOOPS = 4;
    DCCPACKET[0].ADRES = 316;
    DCCPACKET[0].STATE = true;

  DCCPACKET[0].LOOPS = 2;
  DCCPACKET[0].ADRES = 99;
  DCCPACKET[0].STATE = true;
...
void CONSTRUCTBYTES() {
  ...
  */
  ...
  //error byte een XOR maken van de adres en databyte
  int i = 0;
  while (i < 8) {
